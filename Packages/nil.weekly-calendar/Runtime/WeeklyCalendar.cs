...
public class WeeklyCalendar : UdonSharpBehaviour
...
#endif
{
    ...
    void InitFromJson(string result, bool fresh, DateTimeOffset now)
    {
        ...
        {
            Debug.LogError($"Invalid data: {data}");
            return;
        }
        if (data.TokenType != TokenType.DataDictionary)
        {
            Debug.LogError($"Expected data to be a dictionary but found {data.TokenType}");
            return;
        }
        this.Data = result;
    ...
    bool ConvertRealTimeToZone(DateTimeOffset time, string zoneName, out DateTimeOffset inZone)
    {
        ...
        {
            Debug.LogError("Found event with missing time zone");
            return false;
        }
        var zone = zonet.DataDictionary;
        if (!zone.TryGetValue("r", TokenType.DataList, out var offsetst))
        {
            Debug.LogError("Found time zone with no offsets");
            return false;
        }
        ...
        for (var i = 0; i < offsets.Count; i += 1)
        {
            ...
            {
                Debug.LogError("Offset is not an object");
                return false;
            }
            ...
            {
                Debug.LogError("Offset has no value");
                return false;
            }
    ...
    bool GetEventTimeForDate(DataDictionary evt, DateTime date, string zoneName, out DateTimeOffset inZone, out DataDictionary scheduled)
    {
        ...
        {
            Debug.LogError("Event has no start time");
            return false;
        }
    ...
    DataDictionary ConvertSchedule(DataList events, DateTimeOffset now)
    {
        ...
        {
            Debug.LogError("Invalid local time zone");
            return times;
        }
        ...
        for (var i = 0; i < events.Count; i++)
        {
            ...
            {
                Debug.LogError($"Event {i} is not an object");
                continue;
            }
            ...
            {
                Debug.LogError($"Event {i} has no time zone");
                continue;
            }
            ...
            {
                Debug.LogError($"Event {i} has invalid time zone");
                continue;
            }

            if (!evt.TryGetValue("name", TokenType.String, out token))
            {
                Debug.LogError($"Event {i} has no name");
            }
            ...
            if (language != null && language.TryGetValue("name", TokenType.String, out token))
            {
                name = token.String;
            }
            ...
            for (int j = 0; j < 8; j++, date = date.AddDays(1))
            {
                ...
                if (scheduled.TryGetValue("name", TokenType.String, out token))
                {
                    dayName = token.String;
                }
                DataDictionary dayLanguage = null;
                if (language != null && GetByDay(language, dayOfWeek, TokenType.DataDictionary, out token))
                {
                    dayLanguage = token.DataDictionary;
                }
                if (dayLanguage != null && dayLanguage.TryGetValue("name", TokenType.String, out token))
                {
                    dayName = token.String;
                }
                ...
                if (!GetByDay(days, dayOfWeek, TokenType.DataList, out token))
                {
                    Debug.LogError("Day of week does not exist");
                    continue;
                }
                var day = token.DataList;
                ...
                CopyToken(occurrence, "confirmed", evt);
                CopyToken(occurrence, "canceled", evt);
